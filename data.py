...
class QADataset(Dataset):
    ...
    def createPairs_separate2(self):
        ...
        with open(fname, 'r') as f:
            self.trainPairs = pd.read_csv(fname, sep='\t', quotechar="\"", encoding='utf-8', dtype='int')
            print("# of Pairs", len(self.trainPairs))
        return

    def load(self):
        print(os.path.join(self.rootFolder, self.answerFile))
        self.questionFeatures = pd.read_csv(os.path.join(self.rootFolder, self.questionFile), sep='\t',
        ...
        self.concatFeatures = pd.merge(self.questionFeatures, self.answerFeatures, left_on='Id', right_on='ParentId',
                                       suffixes=('_a', '_c')).merge(self.userFeatures, left_on='OwnerUserId_c',
                                                                    right_on='Id', suffixes=('_cx', '_cy')).merge(
            self.userFeatures, left_on='OwnerUserId_a', right_on='Id', suffixes=('_ax', '_ay'))
    ...
    def select(self):
        know_list = []
        for i, j in self.trainPairs.groupby('QuestionId'):
        ...
        self.trainPairs = self.trainPairs[self.trainPairs['QuestionId'].isin(know_list)]
        # select the Pair
    ...
    def filter_time(self):
        know_pairList = []
        for i, j in self.trainPairs_WFeatures.groupby('QuestionId'):
            if "1" in j['Credible'].values:
                know_pairList += j[j['AnswerAge'] >= j[j['Credible'] == '1']['AnswerAge'].values[0]][
                    "PairId"].values.tolist()
    ...
    def add_label(self):
        ...
        for i, j in self.trainPairs.groupby('QuestionId'):
            if "1" in j['Credible'].values:
                ...
                pass
        self.trainPairs['Label'] = self.trainPairs['QuestionId']
    ...
    def change_ID(self):
        ...
        self.mapping = self.trainPairs[["PairId", "QuestionId", "CommentId"]]
    ...
    def filterDataset(self):
        ...
        QuestionIdList = []
        for askerId, content in tempPairMerge.groupby("AskerId"):
            TempList = sorted([(i, j) for i, j in content.groupby("QuestionId")],
                              key=lambda x: x[1]["QuestionAge"].values[0], reverse=True)
            TempList = [(i, "1" in j["Credible"].values) for i, j in TempList]
            for i, j in enumerate(TempList):
                if j[1]:
                    QuestionIdList += [x for x, y in TempList[i:]]
                    break
        QuestionIdList.append("0")
        # the remaining question
        # print QuestionIdList
        ## select ##
        self.trainPairs = self.trainPairs[self.trainPairs['QuestionId'].isin(QuestionIdList)]
        # select the Pair
        Pair_list = self.trainPairs["PairId"].values
        ## select ###
        self.pairFeatures = self.pairFeatures[self.pairFeatures['PairId'].isin(Pair_list)]
    ...
    def createPairs_separate(self):
        ...
        if os.path.isfile(fname):
            ...
            with open(fname, 'r') as f:
                ...
                self.trainPairs.loc[len(self.trainPairs) - 1]["PairId"] = str(len(self.trainPairs))
                ...
                self.pairFeatures.loc[len(self.pairFeatures) - 1]["PairId"] = str(len(self.pairFeatures))

                self.pairText = pd.read_csv(fname2, sep='\t', dtype='str', lineterminator='\n', error_bad_lines=False)
                self.pairText.loc[len(self.pairText)] = ','
                self.pairText.loc[len(self.pairText) - 1]["PairId"] = str(len(self.pairText))
                self.pairText.fillna('', inplace=True)
                # self.filterDataset()
                self.select()
                # self.add_label()
                # print (len(self.trainPairs['QuestionId'].unique()))
                self.trainPairs_WFeatures = pd.merge(self.trainPairs, self.pairFeatures, on='PairId')

                ### change questionAge
                compensate = pd.read_csv(self.rootFolder + "/" + "QuestionFeatures.tsv", sep='\t', dtype='str',
                                         lineterminator='\n', error_bad_lines=False)
                questionAgeDict = dict()
                for i in range(len(compensate)):
                    # print compensate.loc[i,"Id"]
                    questionAgeDict[compensate.loc[i, "Id"]] = compensate.loc[i, "QuestionAge"]
                print(len(questionAgeDict.keys()))

                # print(self.trainPairs_WFeatures.keys())
                self.trainPairs_WFeatures['QuestionAge'] = self.trainPairs_WFeatures['QuestionId'].apply(
                    lambda x: int(questionAgeDict[x]))
                self.trainPairs_WFeatures['QuestionAge'] = self.trainPairs_WFeatures['QuestionAge'].apply(
                    lambda x: int(x))

                ### change answerAge
                compensate = pd.read_csv(self.rootFolder + "/" + "AnswerFeatures.tsv", sep='\t', dtype='str',
                                         lineterminator='\n', error_bad_lines=False)
                answerAgeDict = dict()
                for i in range(len(compensate)):
                    answerAgeDict[compensate.loc[i, "Id"]] = compensate.loc[i, "AnswerAge"]
                self.trainPairs_WFeatures['AnswerAge'] = self.trainPairs_WFeatures['CommentId'].apply(
                    lambda x: int(answerAgeDict[x]))
                self.trainPairs_WFeatures['AnswerAge'] = self.trainPairs_WFeatures['AnswerAge'].apply(lambda x: int(x))

                self.trainPairs_WFeatures["QASpan"] = self.trainPairs_WFeatures['QuestionAge'] - \
                                                      self.trainPairs_WFeatures['AnswerAge']
        ...
        for idx in range(len(self.concatFeatures)):
            ...
            fout.write(str(count) + "\t" + str(asker) + "\t" + str(commenter) + "\t" + str(quesId) + "\t" + str(
                commentId) + "\t")
            fout2.write(str(count) + "\t" + str(location_asker) + '\t' + str(aboutme_asker) + "\t" + str(
                location_commenter) + '\t' + str(aboutme_commenter) + '\t' + str(pair["Body_a"]) + "\t" + str(
                pair["Body_c"]) + '\t' + str(title) + "\n")

            feature_columns = ["Reputation_ax", "LastAccessDate_ax", "Views_ax", "NetworkAge_ax", "Age_ax",
                               "UpVotes_ax", "DownVotes_ax", "Gold_ax", "Silver_ax", "Bronze_ax", "Questions_ax",
                               "Answers_ax",
                               "Reputation_ay", "LastAccessDate_ay", "Views_ay", "NetworkAge_ay", "Age_ay",
                               "UpVotes_ay",
                               "DownVotes_ay", "Gold_ay", "Silver_ay", "Bronze_ay", "Questions_ay", "Answers_ay",
                               "Score_a", "Tags",
                               "Score_c", "QuestionAge", "ViewCount", "AnswerCount", "CommentCount_a", "AnswerAge",
                               "CommentCount_c"]
    ...
    def getAdjList_Similarity1(self):
        ...
        # file = os.path.join(self.rootFolder, 'edges_line.pkl')
        ...
        firstPart = trainPairs.groupby(["CommenterId", "QTag_vector"])
        secondPart = trainPairs.groupby(["CommenterId"])

        # group by CommenterId
        for index in secondPart:
            if len(index[1]) >= 5:
                x = index[1].sort_values(by=["AnswerAge"], ascending=True)["PairId"].tolist()
    ...
    def getAdjList_Similarity2(self):
        ...
        trainPairs["QTag_vector"] = trainPairs["QTags"].apply(lambda x: x.split(","))
        trainPairs["QTag_vector"] = trainPairs["QTag_vector"].apply(lambda x: ' '.join(sorted(list(x))))
        self.trainPairs_WFeatures["ArrivalRank"] = self.trainPairs_WFeatures["QASpan"]
        import scipy.stats as ss
        for i, j in self.trainPairs_WFeatures.groupby('QuestionId'):
            temp = self.trainPairs_WFeatures.loc[self.trainPairs_WFeatures['QuestionId'] == i, "ArrivalRank"]
            self.trainPairs_WFeatures.at[self.trainPairs_WFeatures['QuestionId'] == i, "ArrivalRank"] = ss.rankdata(
                temp)

        trainPairs["Link"] = [0 for i in range(len(trainPairs))]
        step2 = self.trainPairs_WFeatures.groupby(["QuestionId"])
        idxList1 = []
        idxList2 = []
        for i, j in step2:
    ...
    # Content similarity
    def getAdjList_ContentSimilarity(self):

        file = os.path.join(
            "/home/knarang2/stackexchange_adj_data/" + self.dataset + "/" + 'edges_' + 'content' + '.pkl')
        if os.path.exists(file):
            edges = pickle.load(open(file, "r"))
            print("similarity6:", len(edges))
            return edges
        ...
        dir = "/home/knarang2/StackExchangeDataEmbed/" + self.dataset
        embeddings = np.load(dir + "/dataEmbeddings.npy")
        print("Content Embeddings", embeddings.shape)

        def getCosine(embeddings):
            import sklearn.metrics.pairwise
            cosine = np.zeros((len(embeddings), 1))
            # import pdb; pdb.set_trace()
            for i in range(len(embeddings)):
                cosine[i] = sklearn.metrics.pairwise.cosine_similarity(embeddings[i, :50].reshape(1, -1),
                                                                       embeddings[i, 50:].reshape(1, -1))[0]

            return cosine

        cosine = getCosine(embeddings)
        trainPairs["Cosine"] = cosine

        self.Cosine = trainPairs[["PairId", "Cosine"]]

        # create clique using cosine
        step2 = trainPairs.groupby(["QuestionId"])
        idxList1 = []
        idxList2 = []
        for i, j in step2:
            tempScore = sorted(j["Cosine"].values.tolist())
            if tempScore[-1] - tempScore[-2] >= margin:
                idxList1.append(j["Cosine"].idxmax())
            if tempScore[1] - tempScore[0] >= margin:
                idxList2.append(j["Cosine"].idxmin())
        trainPairs.loc[idxList1, "Link"] = 1
        trainPairs.loc[idxList2, "Link"] = 2
        trainPairs["Credible"] = trainPairs["Credible"].apply(lambda x: int(x))
        step3 = trainPairs[trainPairs["Link"].isin([1, 2])].groupby(["CommenterId", "Link"])
    ...
    def getTextFeatures(self, X):
        ...
        Text_Features_df = pd.DataFrame(Text_Features,
                                        columns=['PairId', '#ParaCount', 'Code', '#Words', '#AParaCount', 'ACode',
                                                 '#AWords', "#Title", "#Q_Aboutme", "#A_Aboutme"], dtype='float')
